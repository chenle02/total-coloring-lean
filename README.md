...
### Minimum-six typed CNF semantics (unmerged proof branch)

Proof branch `agent/minsix-threshold-formula` adds a solver-independent typed
CNF layer. `MinSixCNFCore` proves the Boolean meaning of the four threshold
cell shapes and the Sinz at-most-one block. `MinSixThresholdEncoder` assembles
the bounded threshold table and proves
`Threshold.extension_satisfies_exactCountCNF`: when the first `n` primary
inputs contain exactly `k` true values, their canonical threshold auxiliaries
satisfy the typed exact-count formula. The companion `_iff` theorem is only
about that canonical extension.

This branch does not enumerate the 1,560 cage descriptors, assign DIMACS
integers, identify a generated byte stream, parse or check an LRAT proof,
prove a concrete CNF unsatisfiable, compose a semantic blocker with a checked
certificate, or prove any graph-coloring theorem. Every proposed source tree
requires its own external exact-tree receipt before merge.
